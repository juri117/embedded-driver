...
typedef struct {
  unsigned int info, src, dst, length, stride, next, pad[2];
} dma_cb_t;
...
// Default CS word
#define DMA_CS_CONFIGWORD \
  (8 << DMA_CS_PANIC_PRI) | (8 << DMA_CS_PRIORITY) | (1 << DMA_CS_WAIT_FOR)
...
// Default TI word
#define DMA_TI_CONFIGWORD                                 \
  (1 << DMA_TI_NO_WIDE_BURSTS) | (1 << DMA_TI_SRC_INC) |  \
      (1 << DMA_TI_DEST_DREQ) | (1 << DMA_TI_WAIT_RESP) | \
      (1 << DMA_TI_INTEN) | (DMA_DREQ_PWM << DMA_TI_PERMAP)
